...
public partial class GenericReportDataPage : ContentPage
{
	//public static GenericReportData? Data;

	public GenericReportDataPage(GenericReportData Data)
	{
        InitializeComponent();
        _ = DisplayAlert("Welcome", $"The year is {Data.Year} and the supplier name is {Data.SupplierName}", "OK");
		financial_year.Text=Data.Year;
		 Supplier_name.Text=Data.SupplierName;
		var Service = new MainViewModel();
		BindingContext= this;

        var peopledata = Service.databaseService.GetPeople(Data.Year, Data.SupplierName);
        peopleGrid.ItemsSource = peopledata;
    ...
    private async Task GeneratePDFAsync()
    {
        ...
        document.Info.Title = "Generic Report";
        ...
        // Draw Financial Year
        gfx.DrawString($"Financial Year: {financial_year.Text}", regularFont, XBrushes.Black, new XRect(40, 100, page.Width, page.Height), XStringFormats.TopLeft);

        // Draw Supplier Name
        gfx.DrawString($"Supplier: {Supplier_name.Text}", regularFont, XBrushes.Black, new XRect(40, 120, page.Width, page.Height), XStringFormats.TopLeft);
        ...
        var data = peopleGrid.ItemsSource as List<Person>;
